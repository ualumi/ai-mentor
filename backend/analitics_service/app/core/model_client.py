...
async def analyze_code(code: str) -> dict:
    ...
    return{'summary': 'Корректный рекурсивный факториал с проверкой базового случая.', 
           'tags': [{'name': 'Recursion', 
                     'weight': 0.5, 
                     'applied': True, 
                     'score': 9, 
                     'evidence': 'Функция использует рекурсию для вычисления факториала.'}, 
                     {'name': 'Math', 'weight': 0.3, 'applied': True, 'score': 8, 'evidence': 'Вычисляет n! через повторное умножение.'}, 
                     {'name': 'CodeQuality', 'weight': 0.2, 'applied': True, 'score': 9, 'evidence': 'Проверка базового случая присутствует.'}], 
                     'suggested_tags': [], 'overall_score': 9, 'code_quality_score': 8.5, 
                     'correctness': {'is_correct': True, 'score': 9, 'edge_cases_handled': True}, 
                     'task_compliance': {'is_relevant': True, 'score': 9, 'description': 'Рекурсия реализована корректно, возвращает 120 при n=5.', 'missing_requirements': [], 'extra_features': ['Базовый случай', 'Читаемый код'], 'wrong_approach': None}, 
                     'complexity': None, 'strengths': ['Базовый случай предотвращает бесконечную рекурсию', 'Верный результат для положительных чисел', 'Структура легко расширяема на отрицательные значения'], 
                     'weaknesses': ['Не обрабатывает отрицательные входы', 'Не проверяет тип входа', 'Рекурсия может быть затянутой на больших n'], 
                     'recommendations': [f'ra{random.randint(1, 100)}','Добавить проверку на отрицательные значения', 'Использовать int для входа', 'Возможен переполнение стека при больших n'], 
                     'detailed_analysis': 'Решение демонстрирует правильное понимание рекурсии. Функция factorial вызывает саму себя с аргументом n-1 до тех пор, пока n не станет равным 0. Это корректная реализация рекурсии для вычисления факториала.\n\nСильные стороны — правильная логика и проверка базового случая. Рекурсия в данном случае эффективна и легко расширяется для работы с отрицательными числами, хотя это не реализовано здесь.\n\nСлабые стороны — отсутствие проверок на отрицательные значения и типы входа. Это приведет к ошибкам при вызове на отрицательных числах или неправильных типах данных.\n\nДля улучшения рекомендуется добавить проверку на тип входа и обработку отрицательных значений, чтобы функция была более надежной и корректной.'}
    return{"analysis": {
