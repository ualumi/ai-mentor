...
//last norm but max debth useeffect
{/*import { MessageCircleCode, BookMarked, ChevronDown } from 'lucide-react';
import { useEffect, useState, useRef } from 'react';
import { wsService } from '../../services/websocket';
import "../../App.css";
...
export default function Recommendation({ mode, attempt }) {
  ...
  // 🔥 фикс: всегда актуальный activeTab (без пересоздания handler)
  const activeTabRef = useRef(activeTab);
  useEffect(() => {
    activeTabRef.current = activeTab;
  }, [activeTab]);
    ...
    const handler = (data) => {
      ...
      if (data.source?.startsWith("user_progress")) {
        ...
        if (data.data?.recommendations) {
          ...
          if (activeTabRef.current !== "recommendation") {
            setHasNewRecommendation(true);
          }
      ...
      if (data.source?.startsWith("mentor_response")) {
        ...
        // 🔥 критично: раскрываем при первом ответе
        setIsCollapsed(false);

        if (activeTabRef.current !== "mentor") {
          setHasNewMentorReply(true);
        }
      }
    };
  ...
  //    setMentorReplies(() => {
        // 1. если есть hints (старый формат)
  //      if (attempt.analysis?.hints?.length) {
  //        return attempt.analysis.hints.map(h => ({
  //          text: h,
  //          time: new Date(attempt.timestamp).toLocaleTimeString()
  //        }));
  //      }

        // 2. если есть mentor_reply (новый формат)
  ...
  //    setScore({ score: attempt.total_score });
  ...
  useEffect(() => {
    //if (mode === "history" && attempt) {
      ...
      // score
      if (attempt.analysis?.correctness) {
        setScore({
          score: attempt.analysis.correctness.score
        });
      }
